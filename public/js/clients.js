window.onload = (ev) => {
    ...
    const singleItem = function (item) {
      return ` 
           <tr dbid=${item.ID}>
                ...
                <td>${item.DISCOUNT}</td>
                <td>
                    <svg dataid="${item.ID}"  id="edit" xmlns="http://www.w3.org/2000/svg" width="40" style="color: lightseagreen;" height="25" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
  ...
  const filterList = function () {
    ...
    const ReactFxn = function (r, u) {
      deliverPost('/v1/api/registration/filterlist', 'POST', {
        search: r,
        filter: u,
      })
        .then((res) => {
  ...
  var deleteFxn = () => {
    const deleteServer = function (e) {
      ...
      deliverPost('/v1/api/delete/client', 'POST', {
        id,
      })
        .then((res) => {
          console.log(res)
  ...
  const searchInfo = function (id) {
    deliverPost('/v1/api/registration/edit', 'POST', {
      id,
    })
      .then((res) => {
        return res.json()
  ...
  const editFxn = function () {
    ...
    for (let i = 0; i < el.length; i++) {
       el[i].addEventListener("click", (ev) => {
          let id = el[i].getAttribute('dataid');
          console.log(id)
          openReg();
          searchInfo(id);
       })
