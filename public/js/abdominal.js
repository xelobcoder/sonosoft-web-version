window.onload = (ev) => {
  ...
  const getUsername = async function () {
    ...
    if (clientid.value != '') {
      ...
      postRequest(url, 'POST', { id: parseInt(clientid.value) })
        .then((r) => {
          if (r) {
            let fullname = r[0]['FULLNAME']
  ...
  const getCurrent = async function () {
    let currentUserID = window.location.href.split('/').pop()
    let element = document.getElementById('clientid')
    element.value = currentUserID
  }
  ...
  let transactionalID = document.getElementById('clientid')
  ...
  let SCANDATA = function () {
    return {
      ...
      right_kidney:
        getKidneySize().right.length + ' x ' + getKidneySize().right.width,
      left_kidney:
        getKidneySize().left.length + ' x ' + getKidneySize().left.width,
  ...
  const inputFill = async function (data) {
    ...
    left_kidney_width.value = parseFloat(
      LT_KIDNEY_SIZE.split('cm x ')[1].split('cm')[0],
    )
    right_kidney_len.value = parseFloat(RT_KIDNEY_SIZE.split('cm x ')[0].trim())
    right_kidney_width.value = parseFloat(
      RT_KIDNEY_SIZE.split('cm x ')[1].split('cm')[0],
    )
    spleenLength.value = parseFloat(SPLEEN_SIZE.split('cm x ')[0].trim())
    spleenWidth.value = parseFloat(
      SPLEEN_SIZE.split('cm x ')[1].split('cm')[0].trim(),
    )
  }
  ...
  const prefillData = function (requestData) {
  ...
  const displayPreset = async () => {
    ...
    const prefillData = async function () {
      ...
      const sendData = async (id, preset) => {
        postRequest('http://localhost:8000/api/v1/presetspecific', 'POST', {
          id: id,
          preset: preset,
        })
          .then((res) => {
    ...
    postRequest('http://localhost:8000/api/v1/presetTitles', 'POST', {
      scan: 'abdominalscan',
    })
      .then((res) => {
        searchShake(res)
