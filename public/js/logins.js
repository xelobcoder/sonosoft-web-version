window.onload = (ev) =>  {
    ...
    const postRequest = async (url,data) => {
        let link = await fetch(url,{method:"POST",headers: {"content-type": "application/json"},body: data});
        return link;
    }
    ...
    if(addUserButton) {
        addUserButton.onclick =  (ev) => {
          ...
          if(username.value == "" || contact.value == "" || password.value == "") {
              ...
              let data = JSON.stringify({username,fullaccess,role, password,edit,register,deleteitem,contact});
